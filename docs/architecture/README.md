...
- [`benchmarking-and-capacity-evidence.md`](./benchmarking-and-capacity-evidence.md) — portable benchmarking strategy: hardware-independent work-efficiency gates, matched reference performance, optional operator/community benchmarks, deployment classes, normalized resource metrics, provenance, and evidence-backed capacity guidance without tying ActivityPods to one hosting vendor.

`scalability.md` asks whether the system remains healthy as load and data grow. `resource-efficiency.md` asks whether the same correct useful work can be done with less compute and infrastructure. `benchmarking-and-capacity-evidence.md` defines how those claims remain valid across heterogeneous cloud and self-hosted hardware. All are required. APDM is one major workstream inside this broader program rather than the complete scalability/performance effort.
