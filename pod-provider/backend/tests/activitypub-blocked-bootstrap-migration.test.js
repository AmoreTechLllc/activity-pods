...
jest.mock('@semapps/activitypub', () => ({
  ...
  ACTIVITY_TYPES: { BLOCK: 'Block', UNDO: 'Undo', FOLLOW: 'Follow', ACCEPT: 'Accept' },
...
describe('blocked collection legacy bootstrap migration marker', () => {
  ...
  test('first successful migration marks completion only after every actor succeeds', async () => {
    ...
    const broker = {
      call: jest.fn(async action => {
        ...
        if (action === 'auth.account.find') return accounts;
        return undefined;
      }),
      getLocalService: jest.fn(() => ({ processors: [] }))
    };
    ...
    expect(broker.call).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        dataset: 'settings',
        webId: 'system',
        query: expect.stringContaining('urn:activitypods:migration:blocked-collections-v1')
      })
    );
  });
  ...
  test('partial migration failure leaves marker absent so next startup retries', async () => {
    ...
    expect(service.logger.warn).toHaveBeenCalledWith(
      '[activitypub.blocked] legacy collection migration remains incomplete',
      { failureCount: 1 }
    );
  });
});
