...
// Verifies the FEP-3B86 after-hook on the WebFinger service:
//   - merges intent links into res.links on success
//   - degrades silently when the intent service is unavailable
//   - does not error when the upstream WebFinger response is a 404 / null
...
// The mixin in @semapps/webfinger executes at require time; stub it so we can
// load `services/core/webfinger.js` in a unit test without pulling the rest of
// the SemApps stack.
...
const intentLinks = [
  { rel: 'https://w3id.org/fep/3b86/Follow', template: 'https://pod.example/intents/follow?object={object}&on-success={on-success}&on-cancel={on-cancel}' }
];

describe('webfinger FEP-3B86 hook', () => {
  const hook = webfinger.hooks.after.get;
  // Bind to a logger-bearing host so `this.logger.debug` works in the catch path.
  ...
  it('appends intent links to a successful WebFinger response', async () => {
    ...
    const res = { subject: 'acct:alice@pod.example', links: [{ rel: 'self', href: 'https://pod.example/alice' }] };
  ...
  it('returns the original res when res is null/undefined (404 path)', async () => {
    const ctx = makeCtxWithIntents(intentLinks);
