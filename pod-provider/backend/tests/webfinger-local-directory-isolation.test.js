...
describe('local WebFinger directory isolation', () => {
  test('resolves an existing local account through a context-independent broker call', async () => {
    ...
    await expect(service.actions.get.call({ settings: { domainName: 'activitypods' }, broker }, ctx)).resolves.toEqual({
  ...
  test.each([
    ...
    await expect(service.actions.get.call({ settings: { domainName: 'activitypods' }, broker }, ctx)).resolves.toBeUndefined();
