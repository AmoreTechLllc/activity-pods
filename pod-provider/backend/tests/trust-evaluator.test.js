...
describe('TrustEvaluatorMiddleware dataset authority', () => {
  test('uses the exact authoritative account username as the trust dataset', async () => {
    ...
    expect(broker.call).toHaveBeenNthCalledWith(1, 'auth.account.findByWebId', { webId: WEB_ID });
    expect(broker.call).toHaveBeenNthCalledWith(2, 'triplestore.query', expect.objectContaining({ dataset: 'alice' }));
  });
