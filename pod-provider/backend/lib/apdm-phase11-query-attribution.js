...
const SAFE_QUERY_OPERATIONS = new Set(['select','ask','construct','describe','insert','delete','load','clear','create','drop','copy','move','add','with']);
const SAFE_AST_TYPE_VALUES = new Set(['query','update','bgp','graph','group','filter','bind','service','optional','union','minus','values','path','operation','expression']);
const SAFE_TERM_TYPES = new Set(['NamedNode','BlankNode','Literal','Variable','DefaultGraph','Quad']);
const SAFE_AST_OPERATORS = new Set(['=','!=','<','>','<=','>=','+','-','*','/','&&','||','!','in','notin','exists','notexists','bound','regex','sameTerm','isIRI','isURI','isBlank','isLiteral','isNumeric','lang','datatype','str']);
const SAFE_AST_KEYS = new Set(['type','queryType','where','patterns','triples','subject','predicate','object','name','termType','value','datatype','language','variables','template','expression','operator','args','values','group','having','order','limit','offset','distinct','reduced','silent','from','prefixes','updates','insert','delete','using','graph','source','destination']);
const TERM_SLOT_PREFIX = Object.freeze({ NamedNode:'IRI', BlankNode:'BNODE', Literal:'LITERAL', Variable:'VAR', DefaultGraph:'DEFAULT_GRAPH', Quad:'QUAD' });

function normalizeWhitespace(value) { return value.replace(/\s+/gu, ' ').trim(); }

function iriRefEnd(query, startIndex) {
  for (let index = startIndex + 1; index < query.length; index += 1) {
    const char = query[index];
    if (char === '>') return index + 1;
    if (/\s/u.test(char) || char === '<' || char === '"' || char === '{' || char === '}' || char === '|' || char === '^' || char === '`') return undefined;
    if (char === '\\') {
...
function normalizeStringQueryShape(query) {
  ...
  while (index < query.length) {
    ...
    if (char === '<') {
      ...
      if (end !== undefined) { index = end; output += '<IRI>'; continue; }
    }
    if (char === '"' || char === "'") {
      const quote = char;
      ...
      while (index < query.length) {
        ...
        if (triple) {
          if (query.slice(index, index + 3) === quote.repeat(3)) { index += 3; break; }
        } else if (query[index] === quote) { index += 1; break; }
        index += 1;
      }
      output += 'LITERAL';
      continue;
...
function createObjectShapeState() { return { termSlots: new Map(), nextByPrefix: new Map() }; }
...
function normalizeQueryObjectShape(value, key = undefined, depth = 0, state = undefined) {
  ...
  if (Array.isArray(value)) return `[${value.map(item => normalizeQueryObjectShape(item, undefined, depth + 1, currentState)).join(',')}]`;
  ...
  const termType = typeof value.termType === 'string' && SAFE_TERM_TYPES.has(value.termType) ? value.termType : undefined;
  const ordered = Object.entries(value)
    .map(([rawKey, child], index) => ({ rawKey, child, safeKey: SAFE_AST_KEYS.has(rawKey) ? rawKey : 'FIELD', index }))
    .sort((a, b) => a.safeKey.localeCompare(b.safeKey) || (a.safeKey === 'FIELD' ? a.index - b.index : a.rawKey.localeCompare(b.rawKey)));

  const entries = ordered.map(({ rawKey, child, safeKey }) => {
    ...
    const childState = safeKey === 'FIELD' ? createObjectShapeState() : currentState;
    return [safeKey, normalizeQueryObjectShape(child, safeKey === 'FIELD' ? undefined : rawKey, depth + 1, childState)];
  });
...
function normalizeQueryShape(query) {
  ...
  if (query && typeof query === 'object' && !Array.isArray(query)) return `OBJECT:${normalizeQueryObjectShape(query)}`;
  return `UNSUPPORTED:${query === null ? 'null' : typeof query}`;
}
function fingerprintQueryShape(query) { return crypto.createHash('sha256').update(normalizeQueryShape(query), 'utf8').digest('hex'); }
function classifyQueryOperation(query) {
  ...
  const match = normalizeStringQueryShape(query).match(/\b(SELECT|ASK|CONSTRUCT|DESCRIBE|INSERT|DELETE|LOAD|CLEAR|CREATE|DROP|COPY|MOVE|ADD|WITH)\b/iu);
...
function createPhase11QueryAttribution(options = {}) {
  ...
  function reportInstrumentationError(error) { try { onInstrumentationError(error); } catch (_ignored) {} }
  function newTrace(ctx) {
    return { version:1, phase:'APDM-P11-A', requestId:(ctx && (ctx.requestID || ctx.id)) || `apdm-p11-${Date.now()}`, caseLabel,
      recipientCount:Number.isFinite(defaultRecipientCount) ? defaultRecipientCount : undefined, startedAt:new Date().toISOString(),
      contextActions:new Map(), lineageOverflowed:false, droppedLineageContexts:0, aggregates:new Map(), totalQueryCalls:0,
      attributedQueryCalls:0, unattributedQueryCalls:0, overflowed:false, droppedCalls:0, pendingDetachedLocalDeliveries:0,
      rootSettled:false, finalized:false };
  }
  ...
  function recordQuery(trace, ctx, query, durationMs, failed, inheritedActionName) {
    ...
    const aggregateKey = `${caller}\u0000${operation}\u0000${shapeHash}`;
    let aggregate = trace.aggregates.get(aggregateKey);
    if (!aggregate) {
      if (trace.aggregates.size >= maxKeys) { trace.overflowed = true; trace.droppedCalls += 1; return; }
      aggregate = { caller, operation, shapeHash, count:0, errorCount:0, totalDurationMs:0, maxDurationMs:0 };
      trace.aggregates.set(aggregateKey, aggregate);
    }
    aggregate.count += 1;
  ...
  function finalize(trace) {
    ...
    const queries = [...trace.aggregates.values()].sort((a,b) => b.count-a.count || b.totalDurationMs-a.totalDurationMs || `${a.caller}:${a.shapeHash}`.localeCompare(`${b.caller}:${b.shapeHash}`));
    const record = { version:trace.version, phase:trace.phase, requestId:trace.requestId, caseLabel:trace.caseLabel,
      recipientCount:trace.recipientCount, startedAt:trace.startedAt, finishedAt:new Date().toISOString(), totalQueryCalls:trace.totalQueryCalls,
      attributedQueryCalls:trace.attributedQueryCalls, unattributedQueryCalls:trace.unattributedQueryCalls, distinctAttributionKeys:queries.length,
      overflowed:trace.overflowed, droppedCalls:trace.droppedCalls, lineageContextCount:trace.contextActions.size,
      lineageOverflowed:trace.lineageOverflowed, droppedLineageContexts:trace.droppedLineageContexts, queries };
  ...
  const localDeliveryObserver = (phase, activity, error) => {
    ...
    if (!trace) return;
    if (phase === 'start') { trace.pendingDetachedLocalDeliveries += 1; return; }
    if (phase === 'finish') { trace.pendingDetachedLocalDeliveries = Math.max(0, trace.pendingDetachedLocalDeliveries - 1); finalize(trace); }
  };
  ...
  const middleware = {
    ...
    localAction(next, action) {
      ...
      return async function apdmPhase11AttributedAction(ctx) {
        ...
        rememberContext(trace, ctx && ctx.id != null ? String(ctx.id) : undefined, actionName);
        const invoke = async () => {
  ...
  return { middleware, outputPath, dispose() {
...
module.exports = { DEFAULT_MAX_CONTEXTS, DEFAULT_MAX_KEYS, DEFAULT_OUTPUT, DEFAULT_QUERY_ACTION, DEFAULT_ROOT_ACTION,
  classifyQueryOperation, createObjectShapeState, createPhase11QueryAttribution, fingerprintQueryShape, iriRefEnd,
  normalizeQueryObjectShape, normalizeQueryShape, opaqueTermSlot, queryFromContext, safeCallerName };